...
conversionButton.addEventListener("click", () => {
    if (input.value > 3999 || /[a-z]/ig.test(input.value)) { //FIRST STATEMENT INVALIDATES LETTERS AND NUMBERS LARGERS THAN 3,999
        input.value = "";
...
//FUNCTION THAT CONVERTS DECIMAL TO ROMAN NUMBERS
function convertToRoman(num) {
    ...
    for (let i = 1; i < numToArray.length + 1; i++) {
      ...
      if (numbers.hasOwnProperty(numToArray[numToArray.length - i])) {
        finalArray.unshift(numbers[numToArray[numToArray.length - i]]);
      }
      else if (numToArray[numToArray.length - i] === 0) {
      }
      else {
        exponent = numToArray[numToArray.length - i] / counter;
        for (let j = 0; j < exponent; j++) {
          finalArray.unshift(numbers[counter]);
        };
      }
      counter *= 10;
    }
